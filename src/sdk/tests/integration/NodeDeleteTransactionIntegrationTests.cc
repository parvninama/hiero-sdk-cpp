...
TEST_F(NodeDeleteTransactionIntegrationTests, CanExecuteNodeDeleteTransaction)
{
  ...
  TransactionResponse nodeCreateTxResponse;
  ASSERT_NO_THROW(nodeCreateTxResponse = NodeCreateTransaction()
  ...
  TransactionReceipt nodeCreateTxReceipt;
  ASSERT_NO_THROW(nodeCreateTxReceipt = nodeCreateTxResponse.getReceipt(getTestClient()));
  ASSERT_TRUE(nodeCreateTxReceipt.mNodeId.has_value());
  const uint64_t newNodeId = nodeCreateTxReceipt.mNodeId.value();

  // When / Then
  TransactionResponse txResponse;

  ASSERT_NO_THROW(txResponse =
                    NodeDeleteTransaction().setNodeId(newNodeId).freezeWith(&getTestClient()).execute(getTestClient()));
